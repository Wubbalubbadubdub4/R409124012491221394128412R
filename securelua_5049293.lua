...
subTitle.Text = "AUTO PARRY, DODGE & WALK"
...
RunService.PostSimulation:Connect(function()
    ...
    for _, part in pairs(ballsFolder:GetChildren()) do
        if part:IsA("BasePart") then
            ...
            local ballRadius = math.max(part.Size.X, part.Size.Y, part.Size.Z) / 2
            local realDistance = centerDistance - ballRadius - activePad 
            ...
            if speedTowardsMe > 0 then
                local velocityBuffer = speedTowardsMe * REACTION_TIME
                local triggerDistance = math.max(activeRange, velocityBuffer)

                if isParryEnabled and realDistance <= triggerDistance then
                ...
                if isDodgeEnabled then
                    local dodgeTrigger = triggerDistance + 8 
                    if realDistance <= dodgeTrigger and realDistance > 5 then
